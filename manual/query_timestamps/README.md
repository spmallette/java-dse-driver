...
In DSE, each mutation has a microsecond-precision timestamp, which
...
This is enabled by default if you're using a DSE version that supports [native protocol](../native_protocol) v3 or
above.
...
* make sure your `DseCluster` uses a `TimestampGenerator`;
