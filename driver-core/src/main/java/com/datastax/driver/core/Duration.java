...
public final class Duration {
    ...
    private Duration(int months, int days, long nanoseconds) {
        ...
        if ((months < 0 || days < 0 || nanoseconds < 0) && ((months > 0 || days > 0 || nanoseconds > 0))) {
            throw new IllegalArgumentException(String.format(
                    "All values must be either negative or positive, got %d months, %d days, %d nanoseconds", months, days, nanoseconds));
        }
        this.months = months;
        this.days = days;
    ...
    private static class Builder {
        ...
        private void validateOrder(int unitIndex) {
            ...
            if (unitIndex <= currentUnitIndex)
                throw new IllegalArgumentException(String.format("Invalid duration. The %s should be after %s",
